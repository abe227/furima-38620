class PurchasesController < ApplicationController
  ...
  def purchase_params
    params.require(:purchase_home_address).permit(:post_code, :prefecture_id, :municiparities, :address, :building_name, :phone_number, :purchase, :price).merge(user_id: current_user.id, item_id: params[:item_id], token: params[:token])
  end
